...
<body style="font-family:Arial; margin: 0 auto; background-color: #f2f2f2;">
	...
	if (isset($_SESSION['id'])) {
		...
		$sql = "USE syntax_squard_bookstore";
		$conn->query($sql);
		...
		echo "<table style='width:100%; margin-bottom: 2rem'>";
		...
		$sql = "SELECT customer.CustomerID, customer.CustomerName, customer.CustomerIC, customer.CustomerGender, customer.CustomerAddress, customer.CustomerEmail, customer.CustomerPhone, book.BookTitle, book.Price, book.Image, `order`.`DatePurchase`, `order`.`Quantity`, `order`.`TotalPrice`
		FROM customer, book, `order`
		WHERE `order`.`CustomerID` = customer.CustomerID AND `order`.`BookID` = book.BookID AND `order`.`Status` = 'N' AND `order`.`CustomerID` = " . $cID . "";
		$result = $conn->query($sql);
		$total = 0;
		while ($row = $result->fetch_assoc()) {
			...
			echo $row['BookTitle'] . "<br>C$" . $row['Price'] . "<br>";
			echo "Quantity: " . $row['Quantity'] . "<br>";
			echo "<a target='_blank' class='button' style='float: right' type='button' name='cancel' href='invoice.php?username=" . $row['CustomerID'] . "'>View Invoice</a>";
			echo "</td></tr>";
			$total += $row['TotalPrice'];
		}
		echo "<tr><td style='background-color: #ccc;'></td><td style='text-align: right;background-color: #ccc;''>Total Price: <b>C$" . $total . "</b></td></tr>";
		echo "</table>";
		echo "</div>";
		echo "<table style='width:100%; clear: both; background: transparent'>";
		echo "<tr style=' background: transparent'>";
		echo "<td>";
		echo '<footer>';
		echo '<div style="text-align: center; font-size: 12px; margin: 20px 0px">';
		echo 'Neel Bhikadiya = 8885049';
		echo 'Harshil Lunagariya = 8968745';
		echo '</div>';
		echo '</footer>';
		echo "</td>";
		echo "</tr>";
	...
	<style>
		header {
			background-color: rgb(80, 78, 172);
